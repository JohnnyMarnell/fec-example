...
def build_notebook() -> None:
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Jupyter Notebook</h1>
        <p class="text-slate-500 text-sm mt-1">
          Full executed analysis — charts, data joins, and the 60% rule applied.
          Generated by <code class="text-xs bg-slate-100 px-1 rounded">just notebook</code>.
        </p>
      </div>
      <a href="notebook_content.html" target="_blank" rel="noopener"
         class="text-sm text-indigo-600 hover:underline">Open full screen ↗</a>
    </div>
    <div class="rounded-xl overflow-hidden border border-slate-200 shadow-sm" style="height: 80vh;">
      <iframe src="notebook_content.html" class="w-full h-full border-0" title="Analysis notebook"></iframe>
    </div>"""
    else:
        body = """
